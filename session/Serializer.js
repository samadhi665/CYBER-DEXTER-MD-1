...
import { imageToWebp, videoToWebp, writeExifImg, writeExifVid } from '../session/exif.cjs';
import { getBuffer, getSizeMedia } from '../session/myfunc.cjs'
...
function serialize(m, sock, logger) {
  ...
  sock.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
    ...
    if (options.asSticker || /webp/.test(mime)) {
      let { writeExif } = require('./session/exif')
